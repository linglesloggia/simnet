...
class PacketQueueTest(unittest.TestCase):
    ...
    def test_send_pkt(self):
        '''Tests transmission of individual packets and traffic values.

        Tests:
            - reception of packes as string or as int.
            - transmission of packets one by one (no transport block).
            - update of traffic counters sent, received and delay.
        '''
        for i in range(0,3):   # receive packets as string
            self.t_stamp += 1
            self.pkt_count += 1
            self.pkq.receive("DataPkt_" + str(self.pkt_count), self.t_stamp)
            self.pkq.send_pkt(self.t_stamp + 1)
        for i in range(0,3):    # receive packets as int
            self.t_stamp += 1
            self.pkt_count += 1
            self.pkq.receive(40, self.t_stamp)
            self.pkq.send_pkt(self.t_stamp + 1)  # transmits on next time
        ls_sent_vrfy =[
            ['PQ-1:Pkt-1', 1, 2, 'DataPkt_1'],
            ['PQ-1:Pkt-2', 2, 3, 'DataPkt_2'],
            ['PQ-1:Pkt-3', 3, 4, 'DataPkt_3'],
            ['PQ-1:Pkt-4', 4, 5, 40],
            ['PQ-1:Pkt-5', 5, 6, 40],
            ['PQ-1:Pkt-6', 6, 7, 40]]
        self.assertEqual(self.pkq.ls_sent, ls_sent_vrfy)
        # verify traffic dictionary
        nr_pkts = 6                         # number of packets
        nr_bits = 72*3 + 40*3               # bits in packets
        nr_mean = 336/6                     # mean number of bits in packets
        nr_m2 = (72-56)**2*3 + (40-56)**2*3   # M2
        delay = 6                           # transmission delay 1*6
        dc_traf_vrfy = [nr_pkts, nr_bits, nr_mean, nr_m2]
        self.assertEqual(self.pkq.dc_traf["Received"][0:3], dc_traf_vrfy[0:3])
        self.assertAlmostEqual(self.pkq.dc_traf["Received"][3], dc_traf_vrfy[3])
        self.assertEqual(self.pkq.dc_traf["Sent"][0:3], dc_traf_vrfy[0:3])
        self.assertAlmostEqual(self.pkq.dc_traf["Sent"][3], dc_traf_vrfy[3])
        delay = 6                           # transmission delay 1*6
        self.assertEqual(self.pkq.dc_traf["Delay"], delay)
        #print(self.pkq.dc_traf)
        return
