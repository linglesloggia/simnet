...
class Scheduler(Scheduler):
    ...
    def assign_res(self, ls_usreqs, ls_res, time_t):
        ...
        try:
            ...
            # if action is the same as the empty buffer elements, then the reward is 0
            if next_state[action] == 0:
                reward = -10000
            else:
                reward = -np.sum(next_state) #np.sum(1 / (next_state + 1))#-np.sum(next_state)#np.sum(1 / (next_state + 0.001))
        except Exception as e:
            if self.debug:
